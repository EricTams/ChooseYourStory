...
def load_fal_api_key():
    """Load fal.ai API key from various sources."""
    api_key = os.getenv("FAL_API_KEY")
    if not api_key:
        api_key = load_api_key_from_file('fal_api_key.txt')
    return api_key
...
fal_file_api_key = load_api_key_from_file('fal_api_key.txt')
if fal_file_api_key and not os.getenv("FAL_API_KEY"):
    os.environ["FAL_API_KEY"] = fal_file_api_key
...
def generate_image(prompt, api_key=None, api_base=None, background="auto", quality="auto",
    ...
    try:
        ...
        # AIDEV-NOTE: older openai client versions don't support newer params
        for key in ("output_format", "output_compression", "moderation", "background"):
...
def fal_qwen_analyze(input_image, prompt, api_key=None):
    """Analyze an image using fal.ai's Qwen-Image model."""
    
    fal_api_key = api_key or load_fal_api_key()
    
    if not fal_api_key:
        raise ValueError("No fal.ai API key found. Set FAL_API_KEY environment variable or use --fal-api-key")
    ...
    return input_image, result.get("output", "")

def fal_flux_krea_generate(prompt, api_key=None, aspect_ratio="1:1", seed=None, 
                          num_inference_steps=30, guidance_scale=3.5, 
                          enable_safety_checker=False):
    """Generate an image using fal.ai's Flux-Krea model."""
...
def add_api_arguments(parser):
    ...
    parser.add_argument("--provider", type=str, default="openai", choices=["openai", "bfl", "fal"],
    ...
    parser.add_argument("--fal-api-key", type=str, help="fal.ai API key (defaults to FAL_API_KEY env variable)")
...
def generate_command(args):
    ...
    try:
        if args.provider == "fal":
            ...
            result_image = fal_flux_krea_generate(
                prompt=args.prompt,
                api_key=args.fal_api_key,
                aspect_ratio=aspect_ratio,
                seed=getattr(args, 'seed', None),
                num_inference_steps=getattr(args, 'num_inference_steps', 30),
                guidance_scale=getattr(args, 'guidance_scale', 3.5),
                enable_safety_checker=getattr(args, 'enable_safety_checker', False)
            )
        elif args.provider == "bfl":
...
def analyze_command(args):
    ...
    try:
        if args.input:
            ...
            if args.provider == "fal":
                # Use fal.ai Qwen-Image API
                _, description = fal_qwen_analyze(
                    input_image=input_image,
...
def main():
    ...
    # fal.ai-specific arguments
    generate_parser.add_argument("--num-inference-steps", type=int, default=30,
                               help="Number of inference steps (fal.ai only, default: 30)")
    generate_parser.add_argument("--guidance-scale", type=float, default=3.5,
                               help="Guidance scale for generation (fal.ai only, default: 3.5)")
    generate_parser.add_argument("--enable-safety-checker", action='store_true',
                               help="Enable safety checker (fal.ai only, default: False)")
